...
const BroadcasterVideoChat = ({user, setAlert, addBroadcaster}) => {
    ...
    useEffect(() => {
        ...
        window.onunload = window.onbeforeunload = () => {
            axios.delete(`api/broadcasters/${user._id}`)
        ...
        if(user)
        axios.post("api/broadcasters", {streamer: user._id, name: user.firstname + user.lastname, profileimage: user.profileimage || 'avatar1.png'})
            .then((response) => console.log(response))
            .catch((err) => console.log(err));
    ...
    if (stream) {
        UserVideo = (
          <video width="100%" height="100%" playsInline muted ref={userVideo} autoPlay />
        );
    }
    ...
    return (
        <React.Fragment>    
            ...
            <Grid container sx={{display: 'flex', justifyContent: 'space-evenly', flexGrow: 1}}>
                <Button variant="outlined" sx={{mt: 1}} startIcon={<NearMeOutlinedIcon />} onClick={handleBroadcast}>
                    Broadcast
                </Button>
                <Button variant="outlined" sx={{mt: 1}} startIcon={<NearMeDisabledOutlinedIcon />} onClick={handleBanBroadcast}>
                    Ban User
                </Button>
            </Grid>
...
export default connect(mapStateToProps, {setAlert, addBroadcaster})(BroadcasterVideoChat);